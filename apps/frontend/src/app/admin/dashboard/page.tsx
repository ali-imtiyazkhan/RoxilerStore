  ...
  return (
    <DashboardLayout requiredRoles={['SYSTEM_ADMIN']}>
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-1">Overview of platform statistics</p>
      </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900 mt-1">{formatNumber(stats?.totalUsers || 0)}</p>
            </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900 mt-1">{formatNumber(stats?.totalStores || 0)}</p>
            </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900 mt-1">{formatNumber(stats?.totalRatings || 0)}</p>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {Object.entries(stats?.usersByRole || {}).map(([role, count]) => (
            <div key={role} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-slate-600">{role.replace('_', ' ')}</span>
              <span className="text-2xl font-bold text-slate-900">{formatNumber(Number(count))}</span>
            </div>
          ))}
        </div>
      </div>
